            ...
            ${posts
				.map((post) => {
					...
					return `<a href="/post/${post.id}" class="list-group-item list-group-item-action">
                        <img class="avatar-tiny" src="${post.get_user.avatar}"> <strong>${post.title}</strong>
                        <span class="text-muted small">by ${post.get_user.username} on ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
                        </a>`;
				})
				.join('')}
    </div>`);
